(function (angular) {
	...
	.directive('treeModelDelete', ['$compile', function ($compile) {
		return {
			restrict: 'AC',
	...
	.directive('treeModelDownload', ['$compile', function ($compile) {
		return {
			restrict: 'AC',
			link: function (scope, element, attrs) {
				//tree id
				...
				var downloadFunction = 'scope.' + attrs.nodeDownloadFunction || '$scope.download';
				...
				var template =
					...
					'<span data-ng-class="node.selected" data-ng-click="' + treeId + '.selectnodeLabel(node)">{{node.' + nodeLabel + '}}</span>' +
					'</div><div class="col-md-2">' +
					'<button ng-show="node.' + nodeId + '.slice(-1) != '+"'/'"+'" type="button" class="btn btn-success btn-xs pull-right clearfix" aria-label="Left Align" data-ng-click="' + treeId + '.downloadNode(node)"><span class="glyphicon glyphicon-save" aria-hidden="true"></span></button>' +
					'</div></div>' +
					//'<div data-ng-hide="node.collapsed" data-tree-id="' + treeId + '" data-tree-model="node.' + nodeChildren + '" data-node-id=' + nodeId + ' data-node-name=' + nodeLabel + ' data-node-children=' + nodeChildren + '></div>' +
					'</li>' +
					'</ul>';
					...
					if (attrs.angularTreeview) {
						...
						*/
						...
						scope[treeId].selectnodeLabel = scope[treeId].selectnodeLabel || function (selectedNode) {
							...
							//set currentNode
							scope[treeId].currentNode = selectedNode;
							eval(selectFunction)(selectedNode);
						};

						scope[treeId].downloadNode = scope[treeId].downloadNode || function (node) {
							eval(downloadFunction)(node);
						};
					}
